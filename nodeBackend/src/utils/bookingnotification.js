...
const getBrevoApiKey = () =>
  String(process.env.BREVO_API_KEY || '').trim().replace(/^["']|["']$/g, '');
...
const getBrevoErrorMessage = (statusCode, parsed) => {
  ...
  if (lowerMessage.includes('not enabled') || lowerMessage.includes('key not found') || lowerMessage.includes('unauthorized')) {
    return 'Brevo rejected the API key (disabled or invalid). Generate a new v3 key in Brevo → SMTP & API, then update BREVO_API_KEY.';
  }

  if (lowerMessage.includes('ip') && (lowerMessage.includes('unauthorized') || lowerMessage.includes('not authorized'))) {
    return [
      message,
      'Brevo rejected this server IP. Disable authorized IP restrictions or whitelist the server outbound IP.',
    ].join(' ');
  }
...
export const getEmailConfigStatus = () => {
  ...
  if (!getBrevoApiKey()) missing.push('BREVO_API_KEY');
