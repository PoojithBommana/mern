...
export const getPaymentOverview = async (req, res) => {
  try {
    ...
    const [user, summary, transactions, withdrawals] = await Promise.all([
      userModelSchema.findById(userId).select('payoutDetails'),
      getWalletSummary(userId),
      WalletTransaction.find({ userId }).sort({ createdAt: -1 }).limit(15),
      Withdrawl.find({ userId }).sort({ createdAt: -1 }).limit(10),
    ]);
...
export const updatePayoutDetails = async (req, res) => {
  try {
    ...
    const user = await userModelSchema.findByIdAndUpdate(
      req.user.id,
      { payoutDetails },
      { new: true }
...
export const requestWithdrawal = async (req, res) => {
  try {
    ...
    const user = await userModelSchema.findById(userId).select('payoutDetails');
