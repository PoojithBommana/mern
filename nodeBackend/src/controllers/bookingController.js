...
import { cancelBookingCalendarEvent, updateBookingCalendarEvent } from '../utils/googleCalander.js';
  ...
  try {
    ...
    const [bookings, business] = await Promise.all([
      bookingModel.find(query)
        ...
        .sort({ createdAt: -1 })
        .lean(),
      userModelSchema.findById(req.user.id).select('name businessName').lean(),
    ]);
    ...
    const bookingsWithCalendarUrls = bookings.map((booking) => {
      ...
      return {
        ...booking,
        customerCalendarUrl: buildCustomCalendarLink({
          bussiness: business,
...
export const updateBookingStatus = async (req, res) => {
  try {
    ...
    const business = await userModelSchema.findById(req.user.id);
...
export const rescheduleBooking = async (req, res) => {
  try {
    ...
    const booking = await bookingModel.findOne({ _id: req.params.id, userId: req.user.id });
    if (!booking) {
      return res.status(404).json({ message: 'Booking not found' });
    }
    ...
    const hasConflict = conflictingBookings.some((candidate) => (
      checkOverlap(startTime, endTime, candidate.startTime, candidate.endTime)
    ));
    ...
    const [business, service] = await Promise.all([
      userModelSchema.findById(req.user.id),
    ...
    const populatedBooking = await bookingModel.findById(booking._id).populate('serviceId', 'name duration price');
